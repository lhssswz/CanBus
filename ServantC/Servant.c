...
#define MACHINE_ADDRESS           (SLAVE_C_ADDRESS)
...
unsigned char sendTo = SLAVE_C_ADDRESS;
...
unsigned char Sja_1000_Init(void)
{
  ...
  //s=BCAN_SET_OBJECT(0x01,0xe0,0x02,0xe0,0x00,0x0f,0x00,0x0f);//屏蔽寄存器，都设为无关，接收所有报文 
       //当屏蔽位为1，不滤波，0就滤波必须相等
  s=BCAN_SET_OBJECT(MACHINE_ADDRESS,MASTER_ADDRESS,MACHINE_ADDRESS,MASTER_ADDRESS,0X00,0X00,0X00,0X00);
...
/****************************************************
**函数原型：  void AnsData(unsigned char com)
**功    能：  应答数据
**入口参数:   unsigned char com 接收到的命令
**出口参数:   无  
**说    明:   根据接收到的命令做相应的应答          
*****************************************************/
void AnsData(unsigned char com)
{
    switch (com)
    {
      case 0x81:CAN_Send_onebyte(10, ANS_COMMAND_MODULE_STATUS);break;
    //  case 0x82:CAN_Send_onebyte(11, ANS_COMMAND_SET_PARAM);break;
      case 0x83:CAN_Send_onebyte(0, ANS_COMMAND_MODULE_PARAM);break;
      case 0x84:CAN_Send_onebyte(13, ANS_COMMAND_SET_ONOFF);break;
      case 0x85:CAN_Send_onebyte(14, ANS_COMMAND_SET_RESET);break;
      default:break;
    }
    //display(11);
}
