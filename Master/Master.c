...
#define SLAVE_B_ADDRESS           (0x04)
#define SLAVE_C_ADDRESS           (0X08)
...
unsigned char code sendCom_table[] = {SEND_COMMAND_QUERY_STATUS, 
                                      SEND_COMMAND_SET_PARAM, 
                                      SEND_COMMAND_QUERY_PARAM,
                                      SEND_COMMAND_SET_ONOFF,
                                      SEND_COMMAND_SET_RESET};                                      
 ...
 ************************************************************************/ 
 bit   BCAN_ENTER_RETMODEL(void)     //置位复位请求
 {
  ...
  TempData=  *SJA_BCANAdr;       
    *SJA_BCANAdr=0x01;                 //置位复位请求 和单滤波模式
...
void CAN_Send_anylength(unsigned char *CAN_TX_Buf, unsigned char length1, unsigned char com)
{
    ...
    *SJA_BCANAdr = length1 + 1;  
    SJA_BCANAdr  = REG_TXBuffer2;      //本机地址
    *SJA_BCANAdr = sendTo;  
     SJA_BCANAdr = REG_TXBuffer3;     //目标地址
...
void CAN_Send_onebyte(unsigned char CAN_TX_data, unsigned char com)
{
    ...
    SJA_BCANAdr  = REG_TXBuffer1;      //访问地址指向发送缓冲区1,修改成头文件
    *SJA_BCANAdr = 2;  
    SJA_BCANAdr  = REG_TXBuffer2;      //访问地址指向发送缓冲区1,修改成头文件
    *SJA_BCANAdr = sendTo;  
    SJA_BCANAdr  = REG_TXBuffer3;     //访问地址指向发送缓冲区1,修改成头文件
    *SJA_BCANAdr = MACHINE_ADDRESS;
    SJA_BCANAdr  = REG_TXBuffer4;
    *SJA_BCANAdr = com;
    SJA_BCANAdr  = REG_TXBuffer5;
...
unsigned char Sja_1000_Init(void)
{
  ...
  s=BCAN_SET_OBJECT(MACHINE_ADDRESS, 0x0E, MACHINE_ADDRESS, 0x0E,0X00,0x0E,0X00,0x0E);//屏蔽寄存器，都设为无关，接收所有报文 
       //当屏蔽位为1，不滤波，0就滤波必须相等
...
{  
	...
	{  
        ...
        SJA_BCANAdr = REG_RXBuffer5;
      	memcpy(RevceData,SJA_BCANAdr,length - 1);  //功能：由src所指内存区域复制count个字节到dest所指内存区域
			SJA_BCANAdr = REG_RXBuffer3;		//ID2
			switch(*SJA_BCANAdr )
			{
				case 0x02:hongwai = RevceData[0];break;
				case 0x04:yudi = RevceData[0];break;
				case 0x08:shengkong = RevceData[0];break;
				default: break;
			}
...
{  
  ...
  unsigned char key = 0;
	...
	ss=Sja_1000_Init();  //初始化SJA1000     
   
	if (ss!=0)             //初始化失败
	{
		SendStr("Sja_1000_Init() failure");              // 测试专用发送到串口看状态
    display(ss);                                     // 显示错误信息  
	}
   	else
	{
...
// 		// 查询从机状态
// 		sendTo = 0x02;
// 		CAN_Send_onebyte(1,0x83);
// 		delay_ms(20);
// 		sendTo = 0x04;
// 		CAN_Send_onebyte(2,0x83);
// 		delay_ms(20);
// 		sendTo = 0x08;
// 		CAN_Send_onebyte(3,0x83);
// 		delay_ms(20);
// 		
		...
		//执行逻辑
			...
			{
			sendTo = 0x02;
 			CAN_Send_onebyte(2, 0x82);
				}
		
		 if (RevceData[0] == 11)
		{
			sendTo = 0x04;
			CAN_Send_onebyte(3, 0x82);
		}

		 if (RevceData[0] == 12)
		{
			sendTo = 0x08;
			CAN_Send_onebyte(4, 0x82);	
		}
		...
		if (RevceData[0] == 8)
		{
			display(0);
			sendTo = 0x04;
			CAN_Send_onebyte(11, 0x82);	
		}
		if (RevceData[0] == 9)
		{
			display(0);
			sendTo = 0x08;
			CAN_Send_onebyte(12, 0x82);	
		}
...
// 			switch (key)
// 			{
// 				case 0x30: CAN_Send_onebyte(13, sendCom_table[com_select]);break;
// 				case 0x28: sendTo <<= 1; if(sendTo > 0x08 || sendTo < 0x02) sendTo = 0x02; display(sendTo);break;
// 				case 0x18: com_select++; if(com_select > 4) com_select = 0; display(com_select);break;
// 				default:break;
// 			}
// 		}
